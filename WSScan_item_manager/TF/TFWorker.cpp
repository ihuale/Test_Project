...
Status TFWorker::readTensorMulFromMat(const std::vector<ImageCvMeta>* arg_list_mat,
	Tensor & outTensor)
{
...
void TFWorker::setQueues(QQueue<ImageCvMeta>* inQue, ResultMap* outQue)
{
	if (!m_mutex_queue_in) {
		m_mutex_queue_in = new QMutex(QMutex::RecursionMode::Recursive);
	}
	...
	m_inQue = new QQueue<ImageCvMeta>;
	m_inQue = inQue;
	m_list_out = outQue;
	m_flag_queue = true;
